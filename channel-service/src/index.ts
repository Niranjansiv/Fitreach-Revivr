...
type ChannelConfig = {
  ...
  deliveredDelay: [number, number];
  openedRate: number;
  openedDelay: [number, number];
  clickedRate: number;
  clickedDelay: [number, number];
  convertedRate: number;
  convertedDelay: [number, number];
};
...
type CallbackStatus = "DELIVERED" | "OPENED" | "CLICKED" | "CONVERTED" | "FAILED";
...
async function postCallback(payload: CallbackPayload): Promise<void> {
  ...
  try {
    ...
    console.error(`[CHANNEL] Callback error for ${payload.memberName}:`, (err as Error).message);
  }
}

// ── Delivery simulation ───────────────────────────────────────────────────────

async function simulate(send: SendPayload): Promise<void> {
  const cfg = CHANNEL_CONFIGS[send.channel] ?? CHANNEL_CONFIGS.EMAIL;

  const base: Omit<CallbackPayload, "status" | "timestamp"> = {
  ...
  const cb = (status: CallbackStatus): Promise<void> =>
